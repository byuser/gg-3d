// Ported legacy harness — the full headless gameplay suite (formerly the
// bespoke test/harness.js run via `node`), migrated verbatim to Vitest so the
// real ES-module game is exercised end-to-end with NO loss of coverage. Every
// original `ok(cond, msg)` check is preserved as a soft assertion (so the whole
// sequential flow still runs even if one check fails), plus the Task 8 doc-lint.
import { describe, it, expect } from "vitest";
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { Vec3, Color3, makeNode, BABYLON, scenes, handlers, window, localStorage } from "./setup/stubs.js";
import "../src/game.js";

const __dirname = path.dirname(fileURLToPath(import.meta.url));

describe("ported headless harness (parity with the legacy suite)", () => {
  it("runs the full gameplay flow with all legacy checks green", () => {
    const ok = (cond, msg) => { expect.soft(!!cond, msg).toBe(true); };
console.log("\n[1] module load");
...
ok(save && save.v === 7, "serializeGame produced a versioned save");
...
ok(sSave.v === 7 && sSave.story, "save is v7 with a story block");
